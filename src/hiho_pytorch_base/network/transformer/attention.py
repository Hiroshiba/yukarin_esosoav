...
class MultiHeadedAttention(nn.Module):
    ...
    def forward_attention(
        ...
        mask: Tensor | None,  # (B, Td, Te) or (B, 1, Te)
    ):
        """アテンションを計算"""
...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def forward(  # noqa: D102  # type: ignore[override]
        self,
        query: Tensor,  # (B, Td, ?)
        key: Tensor,  # (B, Te, ?)
        value: Tensor,  # (B, Te, ?)
        ...
        score_p = self._rel_shift(score_p)  # (B, H, Td, Td)
